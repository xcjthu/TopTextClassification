...
class JudgePrediction(nn.Module):
    def __init__(self, config):
        ...
        self.predictor = nn.LSTM(2 * config.getint('data', 'hidden_size'), config.getint('data', 'hidden_size'), batch_first = True)
        
        self.out = [nn.Linear(config.getint('data', 'hidden_size'), get_num_class(name)) for name in self.taskName]
    ...
    def forward(self, data, criterion, config, usegpu, acc_result = None):
        ...
        attr, attr_result = self.attr(feature)
        task_vec = self.attention(feature)

        task_vec = torch.cat([task_vec, attr.unsqueeze(1).repeat(1, task_vec.shape[1], 1)], dim = 1)

        task_vec = self.predictor(task_vec)   # batch, taskNum, hidden_size
        
        task_result = {}
        for i in range(len(self.taskName)):
            vec = task_vec[:,i]
            task_result[self.taskName] = self.out[i](vec)

        loss = criterion(attr_result, task_result, labels)
        

        accu = {}
        acc['law'], acc_result['law'] = calc_accuracy(task_result['law'], labels['law'], config, acc_result['law'])
        acc['time'], acc_result['time'] = calc_accuracy(task_result['time'], labels['time'], config, acc_result['time'])
        acc['charge'], acc_result['charge'] = calc_accuracy(task_result['charge'], labels['charge'], config, acc_result['charge'])
