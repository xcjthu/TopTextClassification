...
class ComatchingFormatter4:
    ...
    def check(self, data, config):
        ...
        if "analyse" in data.keys():
            if not (config.getboolean("data", "use_xf")):
                return None
        else:
            if not (config.getboolean("data", "use_other")):
                return None
        if len(data["answer"]) == 0:
            return None
        if len(data["statement"]) == 1:
    ...
    def format(self, data, config, transformer, mode):
        ...
        for temp_data in data:
            ...
            for a in range(0, 4):
                ...
                k = [0, 1, 2, 6, 12, 7, 13, 3, 8, 9, 14, 15, 4, 10, 16, 5, 16, 17]
                for b in range(0, self.k):
                    if config.getboolean("data", "fake_ref"):
                        res.append(self.parseH(["肖", "朝", "军"]))
                    else:
                        res.append(self.parseH(temp_data["reference"][arr[a]][k[b]]))
                document[a].append(res)
