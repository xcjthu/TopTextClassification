...
class Thulac:
    def __init__(self, model_path, user_dict, T2S=False, seg_only=True, filt=False, max_length=50000, deli='_',
                 maxPerSent=500):
        """
        ...
        :param deli:
        """
        self.lac = thulac.thulac(user_dict=user_dict, model_path=model_path, T2S=T2S, seg_only=seg_only, filt=filt,
                                 max_length=max_length, deli=deli)
        self.maxLOneS = maxPerSent

    def Lseg(self, sentence, endOFsentence={"。"}):
        ls = len(sentence)
        idx = 0
        while (idx < ls):
